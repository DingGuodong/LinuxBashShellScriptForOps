...
# Name: release_vux2_frontend.sh
# Execute this shell script to deploy vux2 project(frontend, web static file) built by npm automatically on remote hosts.
...
header="
Function: Execute this shell script to deploy vux2 project(frontend, web static file) built by npm automatically on remote hosts.
...
function git_project_clone(){
    ...
    if test ! -d ${project_clone_directory}; then
        ...
        git clone ${project_clone_repository} ${project_clone_directory} 2>&1 | tee -a ${WORKDIR}/logs/git_$(date +%Y%m%d%H%M).log
        ...
        if test "$current_branch_name" == "$branch"; then
            ...
            git checkout ${branch} 2>&1 | tee -a ${WORKDIR}/logs/git_$(date +%Y%m%d%H%M).log
            git status 2>&1 | tee ${WORKDIR}/logs/git_$(date +%Y%m%d%H%M).log
        fi
        echo_b "git pull from $project_clone_repository with branch or tag name $branch."
        git pull origin ${branch} 2>&1 | tee -a ${WORKDIR}/logs/git_$(date +%Y%m%d%H%M).log
        git status 2>&1 | tee ${WORKDIR}/logs/git_$(date +%Y%m%d%H%M).log
        cd ${WORKDIR}
...
function npm_install(){
    echo_b "Do npm install vux2 project... "
    check_command_can_be_execute cnpm
    [ $# -ge 1 ] && project_clone_repository="$1"
    project_clone_repository_name="`echo ${project_clone_repository} | awk -F '[/.]+' '{ print $(NF-1)}'`"
    project_clone_directory=${WORKDIR}/repository/${project_clone_repository_name}

    cd ${project_clone_directory}
    cnpm install 2>&1 | tee -a ${WORKDIR}/logs/npm_install_$(date +%Y%m%d%H%M).log
    retval=$?
    if [ ${retval} -ne 0 ] ; then
        echo_r "npm install failed! More details refer to $WORKDIR/logs/npm_install_$(date +%Y%m%d%H%M).log"
        exit 1
    else
        cd ${WORKDIR}
        echo_g "Do npm install vux2 project finished for $project_clone_repository_name with exit code $retval. "
        echo
    fi
...
function npm_build_project(){
    echo_b "Do npm build vux2 project for `echo $1 | awk -F '[/.]+' '{ print $(NF-1)}'`... "
    check_command_can_be_execute cnpm
    [ $# -ge 1 ] && project_clone_repository="$1"
    project_clone_repository_name="`echo ${project_clone_repository} | awk -F '[/.]+' '{ print $(NF-1)}'`"
    project_clone_directory=${WORKDIR}/repository/${project_clone_repository_name}
    ...
    cd ${project_clone_directory}
    cnpm run build 2>&1 | tee -a ${WORKDIR}/logs/npm_build_$(date +%Y%m%d%H%M).log
    retval=$?
    if [ ${retval} -ne 0 ] ; then
        echo_r "npm run build for ${project_clone_repository_name} failed! More details refer to ${WORKDIR}/logs/npm_build_$(date +%Y%m%d%H%M).log"
        exit 1
    else
        cd ${WORKDIR}
        echo_g "Do npm run build project finished for ${project_clone_repository_name} with exit code $retval. "
        echo
    fi
...
function ssh_execute_command_on_remote_host(){
    [ $# -ne 2 ] && return 1
    ssh -i /etc/ssh/ssh_host_rsa_key -p 22 -oStrictHostKeyChecking=no root@$1 "$2" 2>&1 | tee -a ${WORKDIR}/logs/ssh_command_$(date +%Y%m%d).log
    retval=$?
    if [ ${retval} -ne 0 ] ; then
...
function make_current_workable_source(){
    ...
    npm_install
    npm_build_project "$user_defined_project_clone"
    cd ${WORKDIR}
...
#        test -f ${WORKDIR}/.capistrano_ds_lock && \rm -rf  ${WORKDIR}/.capistrano_ds_lock
    ...
    if [ -d ${WORKDIR}/repository/${project_clone_repository_name}/dist ]; then
        \cp -rf ${WORKDIR}/repository/${project_clone_repository_name}/dist/* ${new_release_just_created}
    else
