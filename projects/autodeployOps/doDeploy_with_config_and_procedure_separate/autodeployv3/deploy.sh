...
function maven_build_project_deprecated(){
    ...
    project_clone_directory=${WORKDIR}/repository/${project_clone_repository_name}
    cd ${project_clone_directory}
    mvn install >>${WORKDIR}/logs/mvn_build_$(date +%Y%m%d%H%M).log 2>&1
    mvn clean package >>${WORKDIR}/logs/mvn_build_$(date +%Y%m%d%H%M).log 2>&1
    cd ${WORKDIR}
...
function maven_build_project(){
    ...
    retval=$?
    if [ ${retval} -ne 0 ] ; then
        echo_r "mvn install failed! More details refer to ${WORKDIR}/logs/mvn_build_$(date +%Y%m%d%H%M).log"
        exit 1
    else
    ...
    retval=$?
    if [ ${retval} -ne 0 ] ; then
        echo_r "mvn clean package for ${project_clone_repository_name} failed! More details refer to ${WORKDIR}/logs/mvn_build_$(date +%Y%m%d%H%M).log"
        exit 1
    else
...
function maven_install(){
    ...
    cd ${project_clone_directory}
    mvn install >>${WORKDIR}/logs/mvn_install_$(date +%Y%m%d%H%M).log 2>&1
    retval=$?
    if [ ${retval} -ne 0 ] ; then
        echo_r "mvn install failed! More details refer to ${WORKDIR}/logs/mvn_install_$(date +%Y%m%d%H%M).log"
        exit 1
    else
        echo_g "mvn install for ${project_clone_repository_name} successfully! "
    fi
...
function maven_clean_package(){
    ...
    cd ${project_clone_directory}
    mvn clean package >>${WORKDIR}/logs/mvn_clean_package_$(date +%Y%m%d%H%M).log 2>&1
    retval=$?
    if [ ${retval} -ne 0 ] ; then
...
function ssh_execute_command_on_remote_host(){
    [ $# -ne 2 ] && return 1
    ssh -i /etc/ssh/ssh_host_rsa_key -p 22 -oStrictHostKeyChecking=no root@$1 "$2" >>${WORKDIR}/logs/ssh_command_$(date +%Y%m%d).log 2>&1
    retval=$?
    if [ ${retval} -ne 0 ] ; then
...
function make_current_workable_source(){
    ...
    test -z ${user_defined_project_clone_depends} || git_project_clone "$user_defined_project_clone_depends" ${user_defined_project_clone_depends_branch_or_tag}
...
#        fi
        ...
        if [[ "x$branch_from_cli" != "x" ]];then
            branch_or_tag=${branch_from_cli}
...
##            branch_or_tag=${user_defined_project_clone_branch_or_tag}
        ...
        case $1 in
            ...
            destroy)
                ...
                ;;
            help|*)
                echo "Usage: $0 -b <branch name> {deploy|rollback_manual|deploys|backup_manual} with $0 itself"
                echo "Usage: $0 -t <tag name> {deploy|rollback_manual|deploys|backup_manual} with $0 itself"
