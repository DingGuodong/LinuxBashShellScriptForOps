...
if grep -v "trust host ssh key" /root/.ssh/authorized_keys >/dev/null ; then
  {
      echo "trust host ssh key lists, begin"
      ssh-keyscan -t rsa 10.6.28.46 10.6.28.135 10.6.28.27 10.6.28.125 10.6.28.28 10.6.28.35 10.6.28.93 |& awk -F '[ ]+' '!/^#/ {print $2" "$3}'
      echo "trust host ssh key lists, end"
