...
for item in net_connections:
    ...
    if item.status == 'CLOSE_WAIT':
        counter['num_cw'] += 1
print("""Total Connections:{total_conn}
...
print("opened counts for each opened TCP port(incoming) : ", sorted(iter(port_counter.items()), key=lambda x: x[1],
                                                                    reverse=True))  # 'key=lambda x: x[1]' means sort by value

# print opened counts for each opened TCP port, Top 5
tcp_conns_top_5 = sorted(iter(port_counter.items()), key=lambda x: x[1], reverse=True)[:5]  # TCP conns top 5
...
for port, pid in port_pid_mapper.items():
