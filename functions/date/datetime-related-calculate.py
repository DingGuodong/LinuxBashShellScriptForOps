...
Create Time:            17:30
Description:            some useful functions or code snippets related to datetime
Long Description:       
References:             
Prerequisites:          []
...
def compare_two_datetime(left, right, date_format="%Y-%m-%d"):
    """
    排序参数函数，比较两个时间的大小 u1
    sorted(res_raw, cmp=lambda x, y: compare_two_datetime(x, y))

    :param left: date_string
    :type left:str
    :param right:date_string
    :type right:str
...
# cmp to key in Python 3.x
def key_of_specified_datetime(date_string):
    """
    if you prefer to use 'cmp=' rather than 'key=', you can use `from functools import cmp_to_key` to convert it
